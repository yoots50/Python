...
def nextn(x,y) : #x는 주민등록번호, y는 년도 4자리, -다음 판단 함수
    global e # 메세지 중복 방지 변수
    if x[x.find('-')+1:x.find('-')+2].isdigit() == False and x.find('-') != -1  :
        print ('-다음이 숫자가 아닙니다.')
        return 0
    elif e == 'f' :
        return 0
...
def isValid (x) : #x는 주민등록번호, 주민등록번호 판단 함수
    ...
    if len(x) != 14 : #길이 판단
        ...
        error = True
    if len(x) >= 2 and year(x) == 0 : 
        error = True
    if len(x) >= 4 and month(x) == 0 :
        error = True
    if len(x) >= 6 and day(x,month(x),year(x)) == 0:
        error = True
    if len(x) >= 7 and x.find('-') == -1: #- 존재 판단
        print ('-가 없습니다.')
        error = True
        e = 'f' # 메세지 중복 방지 변수
    if e != 'f' and x.find('-') != 6: #-위치 판단
        print ('-가 잘못된 위치에 있습니다.')
        error = True
    if len(x) >= 8 and nextn(x,year(x)) == 0 :
        error = True
    if error == True :
        return False
    else :
